...
<script type="text/javascript">
	...
	function savePM(){
		...
		/*校验控制台输入框*/
		var consoleLoginIsEmpty = checkBlank( $("#consoleLogin").val() );//为空时返回false
		var consolePwdIsEmpty = checkBlank( $("#consolePwd").val() );	 //为空时返回false
		if( consoleLoginIsEmpty && checkCharAndNum( $("#consoleLogin").val() ) && !checkLengthBetween($("#consoleLogin").val() , 3, 20) ){
			$("#msgBoxInfo").html("控制台帐号只允许数字字母,长度3-20位");
			$("#msgBox").modal('show');
			return;
		}
		if( consoleLoginIsEmpty && !consolePwdIsEmpty) {
			$("#msgBoxInfo").html("请输入控制台密码");
			$("#msgBox").modal('show');
			return;
		}
		if(	checkCharAndNum( $("#consolePwd").val() ) && !checkLengthBetween($("#consolePwd").val() , 3, 20) ){
	...
	<div class="up-modal-body">
		<form id="dataForm" class="up-form-horizontal">
			<div class="up-form-group">
				...
				<div class="uup-form-group">
					<div class="up-col-sm-3">
						<select name="ipSegmentId" id="ipSegmentId"
							class="up-form-control" style="width: 200px">
							<option value="">请选择物理机IP网段</option>
							<c:forEach var="serverIp" items="${ipList }">
								<option value="${serverIp.id}" begin="${serverIp.ipBegin }"
									end="${serverIp.ipEnd }">${serverIp.paramName}</option>
							</c:forEach>
						</select>
					</div>
					<label for="" class="up-col-sm-2 up-control-label"> <span
			...
			<div class="up-form-group">
				...
				<div class="up-col-sm-3">
					<input type="text" class="up-form-control" id="consoleLogin"
						name="consoleLogin" placeholder="请输入控制台账号"
						value="${pm.consoleLgoin }">
				</div>
				<label for="" class="up-col-sm-2 up-control-label up-control-label2">
					<span class="up-cq-red-star"> </span>控制台密码
				</label>
				<div class="up-col-sm-3">
					<input type="text" class="up-form-control" id="consolePwd"
						name="consolePwd" placeholder="请输入控制台密码" value="${pm.consolePwd }">
				</div>
			</div>
			...
			<div class="up-form-group">
				...
				<div class="up-col-sm-3">
					<input type="text" class="up-form-control" id="serverLogin"
						name="serverLogin" placeholder="请输入用户账号"
						value="${pm.serverLogin }">
				</div>
				<label for="" class="up-col-sm-2 up-control-label up-control-label2">
					<span class="up-cq-red-star">*</span>用户密码
				</label>
				<div class="up-col-sm-3">
					<input type="text" class="up-form-control" id="serverPwd"
						name="serverPwd" placeholder="请输入用户密码" value="${pm.serverPwd }">
				</div>
			</div>
		</form>
	</div>
	<div class="up-modal-footer up-modal-footer1">
		<button type="button" class="up-btn up-btn-primary" onClick="savePM()">保存</button>
		<button type="button" class="up-btn up-btn-default"
			onClick="parent.window.hideDialog()">返回</button>
	</div>
