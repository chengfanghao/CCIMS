						...
						<div class="up-cq-table-inner">
										...
										<tr>
											...
											<td>
												<c:if test="${vm.isAssigned == 1 }">
													...
													onClick="showDialog('查看虚拟机' , '${basePath}/vmAudit/viewVM?vmOrderId=${vm.id}' , '380px')">查看</a>
												</c:if>
												<c:if test="${orderInfo.state eq 198 and vm.isAssigned == 0 }">
													<a href="javascript:void(0)"
														onClick="showDialog('分配虚拟机' , '${basePath}/vmAudit/doDeployVM?vmId=${vm.id}' , '480px')">分配</a>
												</c:if></td>
										</tr>
									</c:forEach>
								</tbody>
							...
							<div class="up-modal-footer up-modal-footer1">
								<button type="button" class="up-btn up-btn-reset mar-rig30"
									onClick="window.location.href='${basePath}/vmAudit/orderList'">返回</button>
							</div>
						</div>
					</div>
				</form>
